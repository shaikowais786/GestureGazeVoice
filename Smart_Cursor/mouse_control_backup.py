...
def start_mouse_control(show_window=True):
    ...
    while running:
        ...
        try:
                ...
                if smooth_ear < dynamic_threshold:
                    ...
                    if blink_start != 0:
                        ...
                        if blink_locked_pos is not None:
                            # Clamp to ensure we never touch absolute corners
                            safe_margin = 10
                            target_lock_x = max(safe_margin, min(screen_w - safe_margin, blink_locked_pos[0]))
                            target_lock_y = max(safe_margin, min(screen_h - safe_margin, blink_locked_pos[1]))
                            pyautogui.moveTo(target_lock_x, target_lock_y)
                            prev_x, prev_y = target_lock_x, target_lock_y
                            if log_file:
                ...
                if blink_state != "Blinking" and blink_locked_pos is None and post_click_freeze_frames == 0:
                    if max_x - min_x > 0.01 and max_y - min_y > 0.01:
                        ...
                        if abs(diff_x) > 1.5 or abs(diff_y) > 1.5:
                            ...
                            # Clamp to ensure we never touch absolute corners
                            safe_margin = 10
                            prev_x = max(safe_margin, min(screen_w - safe_margin, prev_x))
                            prev_y = max(safe_margin, min(screen_h - safe_margin, prev_y))
            ...
            hand_detected = (hand_loss_frames <= 12)
