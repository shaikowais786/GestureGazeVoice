...
def save_screenshot(frame, reason):
    ...
    # Work with a copy of the frame to avoid any modifications or empty content issues
    ...
    try:
        ...
        print(f"[ERROR] save_screenshot failed: {e}")
...
def launch_app():
    ...
    try:
        if name == "calculator":
            ...
            if local_appdata:
                search_paths.append(os.path.join(local_appdata, r"Google\Chrome\Application\chrome.exe"))
                    ...
                    try:
                        with winreg.OpenKey(hive, r"SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe") as key:
                            ...
                            if val:
                                ...
                                if os.path.exists(val_cleaned):
                                    search_paths.append(val_cleaned)
